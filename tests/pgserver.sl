// A PostgreSQL server written in slate, for the tests that need a real one.
//
...
//
// **The messages are written out longhand from the protocol.** What passes is `api/postgres.sl`
// agreeing with the wire rather than with whichever PostgreSQL happens to be installed, and the suite
// needs no server, no cluster and no password. `pg` keeps its `wire.sl` private -- a consumer imports
// the client and not the message layout -- so this is written from the message formats rather than
// borrowed.
//
// **This arrangement is `sluice`'s `tests/pgserver.sl`**, taken as a shape rather than as a
// dependency: a suite that imported another package's tests would be pinned to a file that package is
// free to change, and neither publishes one.
//
