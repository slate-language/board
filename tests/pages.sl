...
A_THREAD_CARD_CARRIES_ITS_LINK_ITS_BYLINE_ITS_TAGS_AND_ITS_COUNT()
    ...
    assert(contains(markup, "class=\"who\" href=\"/people/ada\""))
    assert(contains(markup, "<a class=\"tag\" href=\"/?tag=slate\">slate</a>"))
...
THE_TAG_BEING_FILTERED_BY_CLEARS_ITSELF_AND_THE_OTHERS_SET_THEMSELVES()
    ...
    assert(contains(markup, "<li class=\"on\"><a class=\"refine\" href=\"/\">slate"))
...
THE_PAGER_COUNTS_PAGES_AND_STOPS_AT_BOTH_ENDS()
    ...
    assert(contains(markup, "<span class=\"step off\">previous</span>"))
...
A_REPLY_LIST_IS_KEYED_AND_IN_THE_ORDER_IT_WAS_GIVEN()
    ...
    assert(indexOf(markup, "first") < indexOf(markup, "second"))
    assert(contains(markup, "<ol class=\"replies\">"))
...
A_BOUNDARY_KEEPS_THE_REST_OF_THE_PAGE_WHEN_THE_THREAD_VIEW_CANNOT_RENDER()
    ...
    assert(contains(markup, "class=\"board-head\""), "and the rest of the page is still there")
    assert(contains(markup, "class=\"board-foot\""))
...
SOMEBODY_WITH_NO_PICTURE_GETS_THE_FIRST_LETTER_OF_THEIR_NAME()
    ...
    assert(contains(markup, "class=\"avatar mono small\" aria-hidden=\"true\">A<"))

    val pictured = shown("/", listing([thread({ author_avatar: "avatars/2/face.svg" })]))

    assert(contains(pictured, "class=\"avatar small\" src=\"/uploads/avatars/2/face.svg\""))
...
@test
THE_THEME_REACHES_EVERY_COMPONENT_THROUGH_ONE_CONTEXT()
    val light = shown("/", listing([]))
    val dark = shown("/", listing([]), { theme: "dark" })

    assert(contains(light, "class=\"board light\""))
    assert(contains(light, ">Dark</button>"), "and the toggle offers the other one")
    assert(contains(dark, "class=\"board dark\""))
...
A_BOOLEAN_ATTRIBUTE_IS_THERE_OR_IS_NOT_THERE_AND_IS_NEVER_FALSE()
    ...
    assert(contains(markup, "required"))
